...
Represent a text headline as the element-wise mean of its word GloVe vectors
...
entire headline into a single vector, losing all word order information.
"""
function embed_review(review::String, glove::Dict{String, Vector{Float32}};
                       dim::Int=100)::Vector{Float32}

    words = _tokenize(review)                               # tokenize the headline text
...
Embed a vector of headline strings into a `(dim, n)` matrix where each column
is the mean GloVe vector for one headline (bag-of-embeddings baseline).
"""
function embed_reviews(reviews::Vector{String}, glove::Dict{String, Vector{Float32}};
                        dim::Int=100)::Matrix{Float32}

    n = length(reviews)
    X = zeros(Float32, dim, n)          # pre-allocate output matrix (dim × n_headlines)
    for i in 1:n
...
Represent a text headline as a padded sequence of word GloVe vectors (instead of a
...
* `X` is `(dim, n_max)`: column `i` holds the GloVe vector for the `i`-th in-vocabulary
  token, or zeros if `i` is past the end of the headline (padding).
...
Headlines longer than `n_max` in-vocabulary tokens are truncated.
"""
function embed_review_sequence(review::String,
    ...
    words = _tokenize(review)                               # tokenize the headline
...
Batch version of `embed_review_sequence`. Embeds a vector of headlines as a 3D tensor
of padded token-vector sequences. Returns `(X, mask, tokens_str)` where:

* `X` is `(dim, n_max, N)` with `N = length(reviews)`.
* `mask` is `(n_max, N)`: true at valid positions for each headline.
...
function embed_reviews_sequence(reviews::Vector{String},
    ...
    N = length(reviews)
    X = zeros(Float32, dim, n_max, N)       # 3D tensor: (embedding_dim, seq_len, n_headlines)
    mask = falses(n_max, N)                  # 2D mask: (seq_len, n_headlines)
    tokens_str = fill("", n_max, N)          # 2D token strings: (seq_len, n_headlines)

    # embed each headline individually and stack into the batch tensors
    for i in 1:N
        Xi, mi, ti = embed_review_sequence(reviews[i], glove; dim=dim, n_max=n_max)
        X[:, :, i] = Xi             # embedding matrix for headline i
        mask[:, i] = mi             # mask vector for headline i
        tokens_str[:, i] = ti       # token strings for headline i
    end
